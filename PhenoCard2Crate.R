...
# Reads a validated PhenoCard JSON-LD file, packages the Profiles
# and related json files, and writes an RO-Crate metadata.
#
...
# Pure content builder — no file I/O — so it can be tested/reused independently
# of where the resulting markdown is written.
build_minimar_content <- function(pc, profile_id, cohort_files) {
  ...
  if (!is.null(inline_agg)) {
    ...
    if (length(age_item) > 0) {
      age_info <- paste0("Mean: ", age_item[[1]]$metric_mean_or_count,
...
# Determine the MINIMAR output filename, write the markdown to disk, and
# return the filename for RO-Crate registration.
...
# Using `overwrite = TRUE` safely replaces any defaults generated by rocrateR
crate <- crate |>
...
# ── Package and write the crate to disk ────────────────────────────────────────

bag_path <- crate |> rocrateR::bag_rocrate(path = OUTPUT_DIR)
